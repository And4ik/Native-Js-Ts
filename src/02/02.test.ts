...
// 1. cоздайте тип CityType
// 2. заполните обьект City, чтобы тесты ниже прошли


test("city should contains 3 houses", () => {
    expect(city.houses.length).toBe(3); //ожидаем, что длина массива равна 3

    expect(city.houses[0].buildedAt).toBe(2012);//ожидаем, что нулевой дом построен в 2012
    expect(city.houses[0].repaired).toBe(false); // ожидаем, что он не разрушен
